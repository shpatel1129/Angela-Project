...
const SvsSQuickAnswer = ({ data, image }) => {
  if (!data) return null;
      ...
      <div className="max-w-[1280px] mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          ...
          <FadeIn direction="right" className="bg-navy-950 text-white rounded-3xl p-8 lg:p-10 relative overflow-hidden group">
            ...
            <span className="font-sans text-[10px] font-bold tracking-[0.25em] uppercase text-ts-gold block mb-4">SILVERSEA CRUISES</span>
            <p className="font-serif text-base text-slate-200 leading-relaxed font-light mb-6">{data.silverseaSummary}</p>
            <div className="border-t border-white/10 pt-5">
              <span className="font-sans text-[9px] font-bold tracking-widest text-slate-400 uppercase block mb-1">BEST FOR</span>
              <p className="font-sans text-xs text-ts-gold font-semibold">Butler Service · Personalized Luxury · S.A.L.T. Culinary</p>
            </div>
          </FadeIn>
          ...
          <FadeIn direction="left" className="bg-slate-50 border border-slate-200/60 text-navy-950 rounded-3xl p-8 lg:p-10 relative overflow-hidden group">
            ...
            <span className="font-sans text-[10px] font-bold tracking-[0.25em] uppercase text-slate-400 block mb-4">SEABOURN</span>
            <p className="font-serif text-base text-slate-600 leading-relaxed font-light mb-6">{data.seabournSummary}</p>
            <div className="border-t border-slate-200 pt-5">
              <span className="font-sans text-[9px] font-bold tracking-widest text-slate-400 uppercase block mb-1">BEST FOR</span>
              <p className="font-sans text-xs text-navy-950 font-semibold">Yacht-Club Atmosphere · Intuitive Service · Intimate Social Vibe</p>
            </div>
          </FadeIn>
        </div>
                      ...
                      <tr key={i} className="hover:bg-white transition-colors">
                        ...
                        <td className="py-4">
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold tracking-wide ${
                            row[1].includes('Silversea') && !row[1].includes('Seabourn') ? 'bg-navy-950 text-white' :
                            row[1].includes('Seabourn') && !row[1].includes('Silversea') ? 'bg-slate-200 text-navy-950' :
...
export default SvsSQuickAnswer;
