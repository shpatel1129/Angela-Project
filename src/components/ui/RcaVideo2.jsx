...
const RcaVideo2 = ({ youtubeId, title, description }) => {
    ...
    <section className="w-full py-20 bg-white text-navy-950 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none">
        <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <circle cx="10%" cy="50%" r="150" stroke="#101d2c" strokeWidth="1" fill="none" />
          <circle cx="90%" cy="50%" r="250" stroke="#101d2c" strokeWidth="1" fill="none" />
        </svg>
      </div>
      ...
      <div className="max-w-[800px] mx-auto px-6 relative z-10 text-center">
        ...
        <FadeIn className="mb-10">
          <span className="font-sans text-[10px] font-bold tracking-[0.3em] uppercase text-ts-gold block mb-3">FLEET IN ACTION</span>
          <h3 className="font-display text-2xl sm:text-3xl font-bold leading-tight text-navy-950 mb-4">
            {title || "Luxury Onboard Tour"}
          </h3>
          <p className="font-sans text-xs sm:text-sm text-slate-650 leading-relaxed font-light max-w-xl mx-auto">
        ...
        <FadeIn delay={0.2} className="mt-8 pt-6 border-t border-slate-100 max-w-xl mx-auto flex items-center justify-between">
          ...
          <span className="font-sans text-[9px] text-ts-gold font-bold uppercase tracking-wider">Seven Seas Splendor Tour</span>
        </FadeIn>
