...
const CreativeShardGrid = ({ title, subtitle, items }) => {
  return (
    <section className="w-full bg-navy-950 py-24 relative overflow-hidden border-y border-white/5">
      ...
      <div className="absolute inset-0 opacity-10 pointer-events-none overflow-hidden flex justify-center items-center">
        <div className="w-[200%] h-32 bg-ts-gold/20 -rotate-12 translate-y-[-200px]"></div>
            ...
            return (
              <FadeIn key={idx} delay={idx * 0.1} className={`w-full ${offsetClasses} h-full`}>
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-ts-gold/0 via-transparent to-ts-gold/0 group-hover:from-ts-gold/10 group-hover:to-transparent transition-colors duration-700"></div>
                  ...
                  <div className={`relative z-10 flex flex-col h-full transform ${unskewDirection}`}>
                    ...
                    <div className="flex justify-between items-start mb-8">
                      <span className="font-display text-6xl text-white/5 font-bold group-hover:text-ts-gold/20 transition-colors duration-500 -ml-2">
                        0{idx + 1}
                      </span>
                      <div className="w-12 h-12 rounded-lg bg-navy-900 border border-white/10 flex items-center justify-center shadow-lg group-hover:border-ts-gold/50 transition-colors duration-300 transform group-hover:scale-110 group-hover:rotate-6">
                    ...
                    <div className="mt-auto">
                      <h3 className="font-display text-2xl text-white mb-4 group-hover:text-ts-gold transition-colors duration-300">
                        {item.title}
                      </h3>
                      <p className="font-sans text-ice-300 text-sm leading-relaxed">
                        {item.description}
                      </p>
                    </div>
                  ...
                  <div className="absolute top-0 right-0 w-8 h-8 bg-ts-gold/30 -z-10 transition-transform duration-500 group-hover:scale-150 rounded-bl-full"></div>
