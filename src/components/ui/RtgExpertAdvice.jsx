...
const RtgExpertAdvice = ({ mistakes, options, image }) => {
  ...
  return (
    <section className="w-full py-20 lg:py-28 bg-stone-50 relative overflow-hidden">
      {/* Background shape */}
      <div className="absolute left-0 top-1/4 w-32 h-64 bg-ts-gold/5 rounded-r-full pointer-events-none"></div>
                  ...
                  <button
                    ...
                    className={`px-4 py-3.5 rounded-2xl font-sans text-xs font-bold transition-all duration-300 border text-center ${
                      selectedId === opt.id
                        ...
                        : 'bg-slate-50 hover:bg-slate-100 text-slate-655 hover:text-navy-950 border-slate-200/60'
                    }`}
                  >
                    {opt.label}
                  </button>
...
export default RtgExpertAdvice;
