...
const WindstarVsLargeDetails = ({ data, tableData, image1, image2 }) => {
  ...
  return (
    <section className="w-full py-20 bg-ice-50 relative overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-6">
        ...
        {/* 2 Big Cards: Yacht-sized vs Megaship itineraries (2,4,6 rule) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden flex flex-col justify-between h-full hover:shadow-lg transition-all duration-300">
              {image1 && (
                <div className="w-full h-56 overflow-hidden relative">
                  <img src={image1} alt="Small yacht-sized itinerary access" className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" loading="lazy" />
          ...
          <FadeIn direction="left">
            <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden flex flex-col justify-between h-full hover:shadow-lg transition-all duration-300">
              {image2 && (
                <div className="w-full h-56 overflow-hidden relative">
                  <img src={image2} alt="Large cruise ship itinerary restrictions" className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" loading="lazy" />
                  <div className="absolute inset-0 bg-gradient-to-t from-navy-950/20 to-transparent"></div>
                </div>
              )}
              <div className="p-8 flex-grow flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="font-display text-2xl text-navy-950 font-bold mb-4">
                    {data.largeDestinations?.title}
                  </h3>
                  <p className="font-sans text-xs sm:text-sm text-slate-500 leading-relaxed">
                    {data.largeDestinations?.description}
                  </p>
                </div>
                <div className="pt-4 mt-6 border-t border-slate-100">
                  <span className="font-sans text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                    Industrial terminals & long bus lines
                  </span>
                </div>
              </div>
            </div>
...
export default WindstarVsLargeDetails;
